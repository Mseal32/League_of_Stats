...
blue_team = ["Player1", "Player2", "Player3", "Player4", "Player5"]
red_team = ["Player6", "Player7", "Player8", "Player9", "Player10"]
...
def show_match_summary(img):
    ...
    with dpg.texture_registry():
        dpg.add_static_texture(width, height, data, tag="Uploaded Texture")
    with dpg.window(label="Match Summary", tag="Match Summary", on_close=cleanup_tags_match_summary):
        with dpg.drawlist(tag="drawlist", height=200, width=200):
            height, width = dpg.get_item_rect_size("Match Summary")
            dpg.draw_image("Uploaded Texture", (0, 0), (height, width), uv_min=(0, 0), uv_max=(1, 1))
    bind_match_summary()
...
def resize_img():
    ...
    dpg.draw_image("Uploaded Texture", (0, 0), (size, size), uv_min=(0, 0), uv_max=(1, 1), parent="drawlist")


with dpg.item_handler_registry(tag="Match Sum Handler"):
    dpg.add_item_resize_handler(callback=resize_img)


def bind_match_summary():
    dpg.bind_item_handler_registry("Match Summary", "Match Sum Handler")
...
def manual_entry_match_summary():
    ...
    with dpg.window(label="Modify Stats", tag="Modify Stats", pos=[700, 100], on_close=cleanup_tags_modify_stats):
        ...
        dpg.add_button(indent=250, label="Apply", callback=lambda: manual_entry_confirm_choices())
